...
export class playerManager {
  ...
  private playerCache: { [id: string]: playerModel } = {};
  ...
  getById(id: string) {
    ...
    if (this.playerCache[id]) {
      this.current$.next(this.playerCache[id]);
    } else {
      // Fetch player from service
      this.playerService.getById(id).subscribe((data) => {
        // Update cache
        this.playerCache[id] = data;
        // Update current$
  ...
  //     this.all$.next(updatedPlayers); // Update all$ BehaviorSubject with the updated array
